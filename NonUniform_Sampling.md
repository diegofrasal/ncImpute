...
x=U%*%(d*t(V)) + E.real
xna=x
#Creating Missing Values in a Full Matrix: STEP #1
totr=n*missfracr
totc=p*missfracc
xna[1:totr,(totc+1):p]=NA
#Creating Missing Values in a Full Matrix: STEP #2
ix=seq(np)
imiss=sample(setdiff(ix,which(is.na(xna))), np*missfracn, replace=FALSE)
xna[imiss]=NA
